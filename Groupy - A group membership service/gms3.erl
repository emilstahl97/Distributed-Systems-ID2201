...
start(Id) ->
    ...
    {ok,
     spawn_link(fun () ->
			init(Id, random:uniform(1000), Self)
		end)}.
...
% extended with the argument N
leader(Id, Master, N, Slaves, Group) ->
    %io:format("~w~n", [N]),
    receive
      {mcast, Msg} ->
	  gms1:leader_bcast(Id, {msg, N, Msg},
			    Slaves), % first to next leader
	  Master ! Msg,
	  ...
	  gms1:leader_bcast(Id,
			    {view, N, [self() | Slaves2], Group2}, Slaves2),
	  Master ! {view, Group2},
...
start(Id, Grp) ->
    ...
    {ok,
     spawn_link(fun () ->
			init(Id, random:uniform(1000), Grp, Self)
		end)}.
...
init(Id, Rnd, Grp, Master) ->
    ...
    Grp !
      {join, Master, self()}, % Grp = someone in the group
    receive
...
slave(Id, Master, Leader, N, Last, Slaves, Group) ->
    receive
	  ...
	  slave(Id, Master, Leader, I + 1, Last2, Slaves2,
		Group2);
      % leader crashed
...
election(Id, Master, N, Last, Slaves, [_ | Group]) ->
    ...
    case Slaves of
	  ...
	  gms1:leader_bcast(Id, Last,
			    Rest), % crucial update: broadcast Last
	  ...
	  erlang:monitor(process,
			 Leader), % if already crashed => 'DOWN'
